...
export default  inject("store")(observer(Reports));
 ...
 function  Reports (props) {
  ...
  const {skill,user,catg,changeCat} = props.store;
...
//         let cd=moment(currentDate).format("D MMM")
...
//             if(d.e.task.length>0){
//               d.e.task.map((t,index,a)=>{
//                 console.log("t: ",t);
...
//            let date=moment(w.D).format("D MMM")
...
//      sArr.push(s)
//      dateArr.push(cd)
//      valArr.push(i % 1 == 0 ? i : i.toFixed(4) )
...
// //  if(pdate  !== date || pdate2 !== date2) {
// //   console.log("user work u c cal : ",user.work)
// //   // setdayLabel(dateArr)
// //   // setdayValue(valArr)
// //   // setdays(sArr)
// //  }
...
//   },[date,date2,skill])
  ...
  const renderTopTab=()=>{
    return(
      <View style={{flexDirection:"row",marginTop:30,marginLeft:5,marginBottom:20}}>
...
Close
  ...
  const renderSkills=()=>{
    ...
    skill.map((item,index,a)=>{
        ts++;
        if(item.e.progress==1){
         cs ++;
         csa.push({name:item.e.name})
        }
        if(item.e.progress<1){
          ics ++;
          icsa.push({name:item.e.name})
         }
      })
