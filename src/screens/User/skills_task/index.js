...
setTimeout(() => {
 ...
 const renderSkillTask=()=>{
  ...
  let task =   item.task.map((item,index,a)=>{
      ...
      if(cat=="pending"?item.submit==true && item.result==null
        ...
        :chk)
        {
          ...
          return(
            ...
            onPress={()=>props.navigation.navigate("Skills_Task_Submit",{taskIndex:index,skillIndex:indexs,from:from})} 
            >
...
<View style={{padding:7,margin:7}}>   
   ...
   <utils.HeaderTitle   title={item.name}  sid={sid}  type="headertitle" />
   </View>
